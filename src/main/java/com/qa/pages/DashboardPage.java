...
public class DashboardPage extends BaseClass{
	...
	public boolean homeMenu() {
		...
		status=TestUtility.clickOn(driver, homeMenuLink,2);
		return status;
	}

	public boolean inventoryModule() {
		boolean status=false;
		status=TestUtility.clickOn(driver, inventory,2);
		return status;
	}
	
	public boolean discussModule() {
		boolean status=false;
		status=TestUtility.clickOn(driver, discuss,2);
		return status;
	}
	
	public boolean manufacturingModule() {
		boolean status=false;
		status=TestUtility.clickOn(driver, manufacturing,2);
		return status;
	}
