...
public class TestUtility extends BaseClass {
	...
	public static boolean clickOn(WebDriver driver, WebElement element, int eleWait) 
	{
		boolean flag=false;
		try {
	...
	public static boolean sendKeys(WebDriver driver, WebElement element,String value,int eleWait ) 
	{
		boolean flag=false;
		try {
			if(element.isDisplayed()) {
	...
	public static boolean sendKeysClean(WebDriver driver, WebElement element,int eleWait) 
	{
		boolean flag=false;
		try {
			if(element.isDisplayed()) {
				...
				for (int i = 0; i <10; i++) {
					...
					Reporter.pass("Clear Text Field");
				}
				flag=true;
			}
		}catch (Exception  e) {
	...
	public static boolean sendKeysWithEncryption(WebDriver driver, WebElement element,String value,int eleWait) 
	{
		boolean flag=false;
		try {
			if(element.isDisplayed()) {
	...
	public static String getText(WebDriver driver,WebElement element, int eleWait) {
		String gettext=null;
		try {
			if(element.isDisplayed()) {
				new WebDriverWait(driver, eleWait).until(ExpectedConditions.visibilityOf(element));
	...
	public static boolean selectValueFromDropDownByText(WebDriver driver,WebElement element, String text,int eleWait) 
	{
		boolean flag=false;
		try {
			if(element.isDisplayed()) {
	...
	public static String getTitlePage(WebDriver driver) {
		...
		Reporter.pass("Get Page title :: "+title);
		return title;
